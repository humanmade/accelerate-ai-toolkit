...
- **Experiment and variant naming convention overhauled — provenance is a badge, not title text.** The skill no longer instructs AI-created tests to carry an `"AI: "` prefix in the experiment or variant title. Provenance is already conveyed by the `source` annotation and rendered as a badge in the UI, so baking "AI:" into the title duplicated the badge and consumed the scannable first characters. The convention is now: titles must be short and legible to a non-developer (the first ~6–7 characters carry the meaning on narrow screens), and must never be derived from the block's H1 (which produced truncated names like `"R2 con…"`). The **experiment title** is a compact iteration code plus the block — `"I4 Hero"`, where `I<n>` is the n-th experiment in the site's auto-research sequence (global across rounds and blocks, tying to the progress chart's experiment number). **Variant titles** name the strategic frame in 1–2 words — `"Control"` for the incumbent, then `"Scarcity"`, `"Outcome"`, `"Social proof"`, `"Safety"`, `"Urgency"` — never `"Variant A/B/C"` and never a headline. The iteration lives on the experiment, so a variant reads in context as `"I4 · Scarcity"`. The `accelerate-learn` journal now tracks an `iteration_counter` (`schema_version` bumped to `3`) so the `I<n>` code stays stable across runs; `accelerate-test` reads it to name the next experiment. All plumbing — abilities, annotations, `source` values — is unchanged; this is naming only.

### Design + evolve engine — generate and optimize variants, not just track tests

Two new skills extend the toolkit from *tracking* tests to *generating and evolving* on-brand designs. No new plumbing — both compose existing capabilities; every mutation is still confirmed (autopilot is the user's standing approval for an unattended loop).

- **New skill `accelerate-design`.** The test-optional primitive: "make me a variant of the hero", "design a new pricing section". It loads the site's full design grammar, composes a bold version from the site's *own* block + pattern vocabulary (not a re-skin), validates it against `docs/design-standards.md`, and *offers* to set it up as a test — never forces one.
- **New skill `accelerate-evolve`.** A multi-round optimization loop on one block: each round fields bold new-structure challengers, tests them, harvests the gate-valid winner, and builds the next round on it. Orchestrates `accelerate-design` + `accelerate-test` (restating none of their mechanics). Runs **attended** (confirm each round) or **autopilot** (unattended, annotated `source: "autopilot"`) — the binding stopping gate and harvest-before-next-round discipline apply in both.
- **New `docs/brand-pack.md` — whole-site design ingest.** Defines the four-layer grammar load (global styles + **all** synced patterns + representative pages across types + media) cached as `brandpack.json` per site. The load-bearing rule: never infer a site's grammar from a single page. `accelerate-test` step 3b now points at it as the canonical loader.
- **New `docs/design-standards.md` §9 — bold-by-default round composition.** One incumbent benchmark + bold new-structure challengers pulled novelty-first from the site's own vocabulary, sized to the traffic budget; cull cleanly-lost concepts and escalate; author a new section only when the vocabulary is spent.
- **`docs/design-standards.md` §14 — the evolve climb: axis ladder + elite crossover.** When new *structures* stop beating the incumbent, the loop escalates the *argument lever* in order — structure → message frame → conversion mechanism → imagery — instead of restyling the same idea; once the site's vocabulary is spent it breeds the next challenger by **crossing the winning traits of ≥2 top performers** (not unrelated novelty), promotes a clean winner back into the brand pack, and **stops at convergence** (K rounds with no gate-valid gain). Validated on a 12-category, 8-round simulation: recomposition plateaus by round 3–6; only an axis-change reliably breaks it.
- **`docs/design-standards.md` §15 — rendering & editability contract.** Beyond per-block validity (§2), whole composed sections are authored to render correctly on *both* surfaces — front end and the block editor: forgiving primitives over fragile `wp:cover`, full-width alignment that survives the editor, deliberate content width, and **first-column parity** (the WordPress block-gap leak that misaligns CSS-gridded rows). Keeps generated sections valid *and* what-you-see-is-what-you-get for editors.
- **Router updated.** Routes the two skills and disambiguates design (one version) vs test (one A/B test) vs evolve (a multi-round loop).

### Concept-driven testing — `accelerate-test` strategy rewrite

The variant-design and experiment-structure guidance now centres on **testing strategic concepts, not element-level levers**. The plumbing is unchanged (abilities, backup ladder, verification steps, annotations) — this is a strategy-layer upgrade that supersedes and integrates the earlier composed-visual, bold-challenger, effect-size, and anti-false-negative additions into one coherent system.
