...
6. **Execute each concept with full visual craft, then check it against `docs/design-standards.md`.** A concept is an *argument staged as an experience* — the imagery, the color pairing, the typography, and the CTA treatment should all serve the frame, not sit on top of it. Score each variant against the differentiation rubric (Message + Visual/structural + Hypothesis clarity, each 0–2). Message Score 2 requires a genuinely different strategic frame (a different motivation appealed to), not the same frame reworded. Visual Score 2 requires a composed redesign — three or more structural levers working together to stage the concept, not one isolated tweak. If the total is below 3/6 or any dimension scores 0, strengthen it — sharpen the frame, compose the execution more boldly, or ground the hypothesis harder in the fetched evidence. Do not present a variant that fails the rubric. For low-traffic sites (under ~1,000 weekly visitors), only propose Score 2 variants.
...
1. Call `accelerate/create-ab-test` with:
   ...
   **Naming convention.** Names are read on small screens and in narrow lists, so the first ~6–7 characters must carry the meaning. Keep both experiment and variant titles short, consistent, and legible to a non-developer. Do **not** bake provenance into the title text: never prefix a title with `"AI: "` or any "AI"/"autopilot" marker. Provenance is conveyed by the `source` annotation (set below) and rendered as a badge in the UI — putting it in the title duplicates the badge and steals the scannable first characters. Do not derive titles from the block's H1 or any heading; a truncated headline (`"R2 con…"`) is not a name.
   ...
   - **Experiment title = a compact iteration code + the block.** Format: `"I<n> <Block>"` — e.g. `"I4 Hero"`, `"I7 Pricing"`. `I<n>` is the n-th experiment in *this site's* auto-research sequence (global across rounds and blocks), so it ties to the experiment number on the site's progress chart and tells a non-developer which iteration they're on. Derive `n` from the site's learning journal iteration counter: `n = (journal `iteration_counter`, or the count of experiments returned by `list-experiments` if no journal exists) + 1`. The journal tracks and increments this counter so the number stays stable across runs (see `accelerate-learn`). Keep it to the code plus the block name — `"I4 Hero"`, not a sentence.
   - **Variant title = the concept in 1–2 words.** Name each arm by its strategic frame, never `"Variant A/B/C"` and never an H1. Use `"Control"` for the incumbent (variant index 0); name challengers by the motivation they argue — e.g. `"Scarcity"`, `"Outcome"`, `"Social proof"`, `"Safety"`, `"Urgency"`. Short, human, consistent. The iteration lives on the experiment, not in every variant — in context a variant reads as `"I4 · Scarcity"`. Good: `"Control"`, `"Outcome"`, `"Social proof"`. Bad: `"Variant B"`, `"AI: R2 con…"`, a truncated headline.
